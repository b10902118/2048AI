...
class feature {
   public:
    ...
    virtual double getTupleValue(int i, board_t j) { return 0; }

    // sign: true-> add, false->minus
    virtual void addFromOtherFeature(feature& ft, bool sign) { return; }
    virtual void addSquaredValueFromOtherFeature(feature& ft) { return; }
    virtual void divideAllDataByN(const int& n) { return; }

    void setSymmetricBoards(board_t* rotateSymmetry, board_t* isomorphic) {
        rotateBoards = rotateSymmetry;
        isomorphicBoards = isomorphic;
    };
    virtual void showContent() { cout << "I'm superclass ... not children" << endl; };

   protected:
    board_t* getRotatedBoards() { return rotateBoards; };
    board_t* getIsomorphicBoards() { return isomorphicBoards; };
