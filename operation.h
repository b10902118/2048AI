...
class operation {
   public:
    static row_t getRow(board_t b, int r) { return (b >> (48 - 16 * r) & 0xffff); }
    ...
    static board_t setCol(board_t b, int c, row_t col) {
        ...
        tempCol = (tempCol & 0xf000) << 48ULL | (tempCol & 0x0f00) << 36ULL |
                  (tempCol & 0x00f0) << 24ULL | (tempCol & 0x000f) << 12ULL;
        tempCol = tempCol >> 4 * c;
    ...
    static board_t setCols(row_t* cols) {
        ...
        for (int i = 0; i < 4; i++) {
            tempCols[i] = cols[i];
            tempCols[i] = (tempCols[i] & 0xf000) << 48ULL | (tempCols[i] & 0x0f00) << 36ULL |
                          (tempCols[i] & 0x00f0) << 24ULL | (tempCols[i] & 0x000f) << 12ULL;
