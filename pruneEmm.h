...
class EMM {
   public:
            ...
            if (player == AI) {
...

                {
                    ...
                    for (int i = 0; i < v.size(); i++) {
                        ...
                        double result = v[i].moveReward + expectiminmax(
                                                              v[i].afterMove, tempAlpha, RANDOM,
                                                              depth - 1, maybeDeadNextCall
                                                          );
                ...
                int empty = 0;
                int sum = configure::rowSum[operation::getRow(checkBoard.m_board, 0)] +
                          configure::rowSum[operation::getRow(checkBoard.m_board, 1)] +
                          configure::rowSum[operation::getRow(checkBoard.m_board, 2)] +
                          configure::rowSum[operation::getRow(checkBoard.m_board, 3)];
                ...
                for (int i = 0; i < 64; i += 4) {
                    if (((checkBoard.m_board >> i) & 0xf) == 0) {
                        ...
                        for (int k = 1; k <= 2; k++) {
                            ...
                            newAlpha = (alpha - accumulatedAlpha - maxSumOfLeftChild) /
                                       (probability[k - 1] / (double)empty);
                            double tempAlpha = -1000000;

                            bool maybeDeadNextCall = false;
                            double result = expectiminmax(
                                (checkBoard.m_board & mask) | (val << i), tempAlpha, AI, depth - 1,
                                maybeDeadNextCall
                            );
                            if (maybeDeadNextCall) maybeDead = true;
    ...
    void setSearchDepth(int depth) { depthLimit = depth; }
        ...
        while (startDepth <= endDepth) {
            if (transTable.exist(originalBoardForTreeSearch, startDepth + 1)) {
                ...
                for (int i = 0; i < v.size(); i++) {
                    ...
                    double result =
                        v[i].moveReward +
                        expectiminmax(v[i].afterMove, alpha, RANDOM, startDepth, maybeDead);
    ...
    void dumpTree() {
        for (int i = 0; i < 10000000; i++) {
            ...
            if (nodes[i].info == "move" || nodes[i].info == "root")
                cout << " best move : " << nodes[i].bestMove;
            cout << endl;
        }
    }
   ...
   private:
    ...
    void selectTuple(const board_t& b) {
        ...
        if (count16kTile > 0 && count8kTile > 0 && count4kTile > 0 && count2kTile > 0)
            iTupleSelect = 4;
