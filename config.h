...
class GameSetting {
   public:
    ...
    static void init_ct_score() {
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 16; j++) {
                configure::ct_score[i * 16 + j] =
                    configure::COMMON_RATIOS[i] * configure::tile_score[j];
            }
        }
    }
    ...
    static void init_score_table() {
        ...
        for (int num = 0; num < 65536; num++) {
            ...
            configure::line_score[num][0] =
                configure::tile_score[line_num[0]] * configure::COMMON_RATIOS[0] +
            ...
            configure::line_score[num][1] =
                configure::tile_score[line_num[3]] * configure::COMMON_RATIOS[4] +
            ...
            configure::line_score[num][2] =
                configure::tile_score[line_num[0]] * configure::COMMON_RATIOS[8] +
            ...
            configure::line_score[num][3] =
                configure::tile_score[line_num[3]] * configure::COMMON_RATIOS[12] +
    ...
    static void init_MinMaxSum() {
        ...
        for (int num = 1; num < 32768; num++) {
            ...
            while (1) {
                ...
                if (val > 0) {
                    board_t addVal = pow;
                    configure::maxScoreOfSum[num * 2] +=
                        configure::tile_score[pow] * configure::COMMON_RATIOS[count];
                    configure::minScoreOfSum[num * 2] +=
                        configure::tile_score[pow] * configure::COMMON_RATIOS[15 - count];
