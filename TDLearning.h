   ...
   public:
    ...
    void learnEvaluation(board_t oBoard, int dir, int reward, board_t mBoard, board_t aBoard) {
        ...
        if (nextDir != -1) {
            ...
            tN[currentTN].updateValue(
                mBoard, LEARNING_RATE * ((double)nextReward +
                                         tN[currentTN].getValue(checkBoard.getCurrentPosition()) -
                                         tN[currentTN].getValue(mBoard))
            );
        } else {
            tN[currentTN].updateValue(
                mBoard, LEARNING_RATE * ((double)nextReward + 0 - tN[currentTN].getValue(mBoard))
            );
        }
    };
    ...
    void learnEvaluation(
        board_t oBoard, int dir, int reward, board_t mBoard, board_t aBoard, board_t mBoard2,
        board_t aBoard2
    ) {
        double firstScore = 0;
        ...
        firstScore =
            (1 - lambda) * (nextReward + tN[currentTN].getValue(checkBoard.getCurrentPosition()));
        ...
        if (!isEnd) {
            ...
            if (nextDir != -1) nextReward = checkBoard.canMove(nextDir);
                ...
                ;
            secondScore =
                lambda * (nextReward + tN[currentTN].getValue(checkBoard.getCurrentPosition()));
        } else
            ;
        ...
        tN[currentTN].updateValue(
            mBoard, LEARNING_RATE * (firstScore + secondScore - tN[currentTN].getValue(mBoard))
        );
    };
    ...
    void updateEvaluation() {
        ...
        for(int i = boardChains[currentTN].size()-1; i >= 0; i--){
                        learnEvaluation(0, 0, 0, boardChains[currentTN][i].moveBoard ,
        boardChains[currentTN][i].addedBoard);
        }*/
        ...
        for (int i = boardChains[currentTN].size() - 1; i >= 0; i--) {
            ...
            tN[currentTN].updateValue(
                boardChains[currentTN][i].moveBoard,
                LEARNING_RATE *
                    (score - tN[currentTN].getValue(boardChains[currentTN][i].moveBoard))
            );
        }
    }
        ...
        for (int i = 0; i < 7000 * 1000 /*&& numberOfBoardsCollected <= 100000*/; i++) {
            ...
            if (iGameCount % 1000 == 0) {
                ...
                cout << "Round: " << iGameCount << "\tAvg. score: " << avgScore / 1000
                     << "\tMax Score: " << maxScore << "\tMax Tile: " << tempMaxTile
                     << "\t#: " << maxCount << "\t#of 16K: " << tile_num[0]
                     << "\t#of 16+8K: " << tile_num[1] << "\t#of 16+8+4K: " << tile_num[2]
                     << "\t#of 16+8+4+2K: " << tile_num[4] << "\t#of 32K: " << tile_num[3] << endl;
        ...
        for (int i = 1; i <= num && numberOfBoardsCollected <= 100000; i++) {
            ...
            if (i % 1000 == 0) {
                ...
                out << "Round: " << i << "\tAvgScore: " << avgScore / 1000
                    << "\tMax Score: " << maxScore << "\tWin Rate: " << (double)winNum * 1.0 / 1000
                    << "\tMax Tile: " << localMaxTile << "\t#: " << maxCount
